 ...
 * 
 * 1/ A partir des criteres envoyes par le client, la base est interrogee pour
 * generer un fichier de donnees XML sur le disque du serveur. 2/ Une "maquette"
 * (un ou plusieurs fichiers .jasper) doit etre fournie a la moulinette
 * JasperReports qui "nourrit" celle-ci avec les donnees contenues dans le
 * fichier xml.
 */
...
public class Editions {
	...
	static public String saveInTempFile(final StringWriter sw,
		...
		// si le fichier existe deja, il est remplacé sauvagement :
	...
	static public String cleanedString(Object obj) {
		...
		tmp.replaceAll("&", "&amp;").replaceAll("<", "\u003C").replaceAll(">", "\u003E")
				.replaceAll("\r\n", "\n").replaceAll("¬ø","\u00BF").replaceAll( "&ugrave;","\u00F9")
				.replaceAll("&egrave;","\u00E8").replaceAll("&ecirc;","\u00EA")
				.replaceAll("&euml;","\u00EB").replaceAll( "&amp;","\u0026").replaceAll( "%","\u0025");
		return tmp;
	}
	...
	static public String cleanKoma(Object obj) {
		if (obj == null)
			return null;
		String tmp = obj.toString().replaceAll("¬ø","'");
		return tmp;
	}
	
	static public String cleanAmp(Object obj) {
		if (obj == null)
			return null;
