...
public class NextProjectCtrl extends ModuleCtrl<NextProject> {
	...
	public WOActionResults fermer() {
		...
		if (ec != null && ec.hasChanges()) {
			 ...
			 * "confirm",
			 * "Des changements n'ont pas été enregistrés, voulez-vous vraiment fermer le projet?"
	...
	// Supression d'une personne sélectionnee dans le tableau dynamique RH
	...
	// Supression d'un poste sélectionné dans le tableau dynamique Recrutement
	...
	public WOActionResults removeMatExt() {
		try {
			...
			// TODO: handle exception
			sess().addSimpleErrorMessage("Supression matériel existant",
					e.getLocalizedMessage());
			wocomponent().context().response().setStatus(500);
			AjaxUpdateContainer.updateContainerWithID(wocomponent().myApp()
					.messageContainerID(), wocomponent().context());
	...
	public WOActionResults removeNewMat() {
		try {
			...
			// TODO: handle exception
			sess().addSimpleErrorMessage("Supression nouveau matériel",
					e.getLocalizedMessage());
			wocomponent().context().response().setStatus(500);
			AjaxUpdateContainer.updateContainerWithID(wocomponent().myApp()
					.messageContainerID(), wocomponent().context());
	...
	public WOActionResults ajoutPoste() {
		try {
			...
			sess().addSimpleErrorMessage("Estimation des personnes à recruter",
					e);
			e.printStackTrace();
		} catch (Exception e) {
			sess().addSimpleErrorMessage("Estimation des personnes à recruter",
	...
	public WOActionResults ajoutMatos() {
		try {
			...
			sess().addSimpleErrorMessage("Estimation des ressources matériels",
					e);
			e.printStackTrace();
		} catch (Exception e) {
			sess().addSimpleErrorMessage("Estimation des ressources matériels",
	...
	public Boolean isTerminerUnProjetDisabled() {
		if (isTerminerUnProjetDisabled == null) {
			...
			String resume = "du resumé du projet";
			String message = "Penser à renseigner au moins une des informations ";
			...
			if (porteur && equipe && (axe || pst) && (ctx || obj || mtd || plm)
				...
				// sess().setAlertMessage("Penser à renseigner toutes les informations nécessaires (+) pour activer le bouton 'Soumettre'");
				wocomponent().context().response().setStatus(500);
				AjaxUpdateContainer.updateContainerWithID(wocomponent()
						.updateNextContainerId(), wocomponent().context());
			}
	...
	public WOActionResults saveOnImplication() {
		...
		if (Double.valueOf(d) > 100) {
			sess().addSimpleErrorMessage("Valeur trop élevée",
					"Le maximum est 100");
	...
	protected BigDecimal coutMoyenRh(String pctge, Integer cm, String mois) {
		...
		NSLog.out.appendln("persRh - Coût estimé à " + tx + "%  sur " + moi
	...
	protected BigDecimal coutMoyenRecrue(String pctge, Integer cm, String mois) {
		...
		eurCostFormat.setMinimumFractionDigits(1);
		eurCostFormat.setMaximumFractionDigits(2);
		NSLog.out.appendln("Poste - Coût estimé à " + tx + "% sur " + moi
				+ " mois : " + eurCostFormat.format(couMoy.doubleValue()));
		return couMoy;
	}
	...
	protected BigDecimal coutNouvoMatos(String pctge, BigDecimal cm) {
		...
		eurCostFormat.setMinimumFractionDigits(1);
		eurCostFormat.setMaximumFractionDigits(2);
		NSLog.out.appendln("Nouveau matériel -" + tx + "% Coût estimé "
	...
	protected void save() {
		...
		if (success)
			traitementMotClef();
		// Lancement de l'insertition des données du tableau dynamique
		if (success)
	...
	// Supression du projet enregistré en cas d'erreur
