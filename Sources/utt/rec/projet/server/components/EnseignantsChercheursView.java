...
import utt.rec.projet.server.components.controler.EnseignantsChercheursViewCtrl;
...
public class EnseignantsChercheursView extends MonComposant {
	/**
	 * 
	 */
	private static final long serialVersionUID = -9072857568649191661L;
	...
	private EnseignantsChercheursViewCtrl ctrl = null;
	...
	public EnseignantsChercheursView(WOContext context) throws ComparisonException {
		super(context);
		NSLog.out.appendln("EnseignantsChercheursView");
		mySession().reset();
		mySession().resetAll();
	 ...
	 * @return the ctrl
	 */
	public EnseignantsChercheursViewCtrl ctrl() {
		if (ctrl == null)
			ctrl = new EnseignantsChercheursViewCtrl(this);
		return ctrl;
	}
	 ...
	 *            the ctrl to set
	 */
	public void setCtrl(EnseignantsChercheursViewCtrl ctrl) {
		this.ctrl = ctrl;
	}
	...
	public String ListidContainerId() {
	...
	public void allProjects() throws ComparisonException {
		if (mySession().getFiltreProjetFacade().allObjects().count() == 0) {
			initFilterEc();
			mySession().setListProjet(
					EOPprec.filtreProjetEC(edc(), mySession().filtres()));
			...
			if (mySession().getBigListTmp() != null) {
				...
				NSArray ordering = new NSArray(
						new Object[] { EOPprec.SORT_DATEMAJ_DESC });
				projetDg().setSortOrderings(ordering);
				projetDg().updateDisplayedObjects();
				mySession().getFiltreProjetFacade().setObjectArray(projetDg().allObjects());
			}
		} else {
			projetDg().setObjectArray(mySession().getFiltreProjetFacade().displayedObjects());
			projetDg().setNumberOfObjectsPerBatch(DEFAULT_DISPLAY);
			NSArray ordering = new NSArray(
					new Object[] { EOPprec.SORT_DATEMAJ_DESC });
			projetDg().setSortOrderings(ordering);
	...
	public void initFilterEc() {
		if (mySession().getSelectedState() == null
				&& mySession().getSelectedProjet() == null
				&& mySession().getSelectedPorteur1() == null
				&& mySession().getSelectedContrat1() == null
				&& mySession().getSelectedRechUnit1() == null) {
			mySession().filtres().takeValueForKey(noIndividu(),
					"noIndiv_Implique");
			mySession().filtres().takeValueForKey(noIndividu(),
					"noIndiv_Porteur");
			mySession().filtres().takeValueForKey(persId(), "perId_CreAuteur");
		}
		mySession().filtres().takeValueForKey(noIndividu(), "dir_Resp");
	}
}