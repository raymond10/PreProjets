...
public class Projects extends MAssistant {
	...
	public Projects(WOContext context) {
		...
		/**
		 * On croise les persId des personnes faisant partie d'une équipe avec
		 * ceux qui sont dans un pole au même instant "t"
		 **/
	...
	// Nouvelles Action suite à modification des droits
	// 03/06/2013
	/**
	...
	public WOActionResults refreshPole() {
		try {
			...
			// TODO: handle exception
			mySession().addSimpleErrorMessage("Sélection porteur",
	...
	public WOActionResults refreshEquipes() {
		try {
			...
			// TODO: handle exception
			mySession().addSimpleErrorMessage("Sélection équipe principale",
					e.getLocalizedMessage());
			context().response().setStatus(500);
			AjaxUpdateContainer.updateContainerWithID(myApp()
					.messageContainerID(), context());
	...
	public WOActionResults onChange() {
		try {
			...
			// TODO: handle exception
			mySession().addSimpleErrorMessage("Sélection équipe sécondaire",
					e.getLocalizedMessage());
			context().response().setStatus(500);
			AjaxUpdateContainer.updateContainerWithID(myApp()
					.messageContainerID(), context());
	...
	public WOActionResults hasThematique() {
		try {
			...
			// TODO: handle exception
			mySession().addSimpleErrorMessage("Sélection thématique",
					e.getLocalizedMessage());
			context().response().setStatus(500);
			AjaxUpdateContainer.updateContainerWithID(myApp()
					.messageContainerID(), context());
	...
	public boolean valider() {
		...
		if (projet != null) {
			...
			if (projet.projduree() == null || projet.projduree().equals(""))
				failureMessages = failureMessages
						.arrayByAddingObject("La durée du projet");
			if (projet.projdatdebut() == null)
				failureMessages = failureMessages
						.arrayByAddingObject("La date de début du projet");
			if (projet.equipePrincipale() == null)
				failureMessages = failureMessages
						.arrayByAddingObject("Le programme Principal d'accueil du projet");
			if (projet.toAxe() == null)
				failureMessages = failureMessages
						.arrayByAddingObject("L'axe de l'équipe projet ou programme scientifique et technologie");
	...
	protected boolean isNumeric(String input) {
		try {
			...
			mySession().addSimpleErrorMessage("Durée du projet",
