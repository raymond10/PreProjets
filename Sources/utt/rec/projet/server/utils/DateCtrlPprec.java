...
public class DateCtrlPprec extends DateCtrl implements
		I_ClasseMetierNotificationParametre {
	 ...
	 * @return
	 */
	public static NSArray joursFeriesEntre2Dates(NSTimestamp firstDayTS,
			NSTimestamp lastDayTS) {
		...
		joursFeries = DateCtrlPprec.holidaysFR(firstDayYear);
		while (firstDayYear < lastDayYear) {
			firstDayYear++;
			joursFeries = joursFeries
					.arrayByAddingObjectsFromArray(DateCtrlPprec
		...
		while (enumJoursFeries.hasMoreElements()) {
			NSTimestamp unJourFerie = (NSTimestamp) enumJoursFeries
					.nextElement();
			if (isAfterEq(unJourFerie, firstDayTS)
					&& isBeforeEq(unJourFerie, lastDayTS)) {
	...
	static public long nbreJoursOuvresEntre(NSTimestamp firstDayTS,
			NSTimestamp lastDayTS) {
		...
		if (isAfter(firstDayTS, lastDayTS)) {
			...
			while (isAfterEq(lastDayTS, currentDayTS)) {
				...
				currentDayTS = currentDayTS.timestampByAddingGregorianUnits(0,
						0, 1, 0, 0, 0);
			}
		}
	...
	public static NSTimestamp toLundi(NSTimestamp date) {
		...
		if (dayOfWeek == GregorianCalendar.SUNDAY) {
			lundiEnCours = lundiEnCours.timestampByAddingGregorianUnits(0, 0,
					1, 0, 0, 0);
		} else {
			lundiEnCours = lundiEnCours.timestampByAddingGregorianUnits(0, 0,
					-((dayOfWeek - 2) % 7), 0, 0, 0);
	...
	public static NSTimestamp dateToDebutAnneeUniv(NSTimestamp uneDate) {
		...
		int jourDebutAnnee = Integer.valueOf(debutAnnee.substring(0, 2))
				.intValue();
		int moisDebutAnnee = Integer.valueOf(debutAnnee.substring(3, 5))
				.intValue();
		...
		NSTimestamp debutAnneeDecalee = uneDateDecalee
				.timestampByAddingGregorianUnits(0,
		...
		NSTimestamp debutAnnee = debutAnneeDecalee
				.timestampByAddingGregorianUnits(0, (moisDebutAnnee - 1), 0, 0,
						0, 0);
		return debutAnnee;
	}
	...
	public static NSTimestamp dateToFinAnneeUniv(NSTimestamp uneDate) {
		return dateToDebutAnneeUniv(uneDate).timestampByAddingGregorianUnits(1,
				0, -1, 0, 0, 0);
	}
	...
	public static NSTimestamp dateToDebutAnneeCivile(NSTimestamp uneDate) {
		...
		NSTimestamp debutAnneeCivile = uneDate.timestampByAddingGregorianUnits(
				0, -nowGC.get(GregorianCalendar.MONTH),
				-nowGC.get(GregorianCalendar.DAY_OF_MONTH) + 1,
				-nowGC.get(GregorianCalendar.HOUR_OF_DAY),
				-nowGC.get(GregorianCalendar.MINUTE),
				-nowGC.get(GregorianCalendar.SECOND));
	...
	public static NSTimestamp date1erJanAnneeUniv(NSTimestamp uneDate) {
		...
		return debutAnneeUniv.timestampByAddingGregorianUnits(0, diffMois,
				-diffJour, 0, 0, 0);
	}
	...
	public static NSTimestamp dateToFinAnneeCivile(NSTimestamp uneDate) {
		return dateToDebutAnneeCivile(uneDate).timestampByAddingGregorianUnits(
				1, 0, -1, 0, 0, 0);
	}
	...
	public static NSTimestamp dateToDebutMois(NSTimestamp uneDate) {
		...
		GregorianCalendar nowGC = new GregorianCalendar();
		nowGC.setTime(uneDate);
		NSTimestamp debutMois = uneDate.timestampByAddingGregorianUnits(0, 0,
				-nowGC.get(GregorianCalendar.DAY_OF_MONTH) + 1,
				-nowGC.get(GregorianCalendar.HOUR_OF_DAY),
				-nowGC.get(GregorianCalendar.MINUTE),
				-nowGC.get(GregorianCalendar.SECOND));
	...
	public static NSTimestamp dateToFinMois(NSTimestamp uneDate) {
		return dateToDebutMois(uneDate).timestampByAddingGregorianUnits(0, 1,
				-1, 0, 0, 0);
	}
	...
	/**
	 * retourne l'annee universitaire d'une date. ex : 10/10/2004 - > 2004/2005
	 * 
	 * @return
	 */
	public static String anneeUnivForDate(NSTimestamp aDate) {
		...
		if (myCalendar.get(GregorianCalendar.MONTH) + 1 >= moisDebutAnnee)
			return myCalendar.get(GregorianCalendar.YEAR) + "/"
					+ (myCalendar.get(GregorianCalendar.YEAR) + 1);
		else
			return (myCalendar.get(GregorianCalendar.YEAR) - 1) + "/"
					+ myCalendar.get(GregorianCalendar.YEAR);
	}

	/**
	 * retourne la date de debut d'annee universitaire pour une periode
	 * universitaire donnee ex : (String)"2004/2005" -> (NSTimestamp)01/09/2004
	 * 
	 * @return
	 */
	public static NSTimestamp dateDebutAnneePourStrPeriodeAnnee(
			String aAnneeUniv) {
		return stringToDate(debutAnnee + "/" + aAnneeUniv.substring(0, 4));
	}

	/**
	 * retourne la date de debut d'annee universitaire pour une annee donnee ex
	 * : (String)"2004" -> (NSTimestamp)01/09/2004
	 * 
	 * @return
	 */
	 ...
	 * methode pour tenir compte aussi des periodes qui n'incluent pas la date
	 * du jour, mais qui sont dans l'annï¿½e universitaire en cours.
	 * 
	 ...
	 */
	public static NSArray lesPeriodesAnneeUniv(NSTimestamp dateRef,
			NSTimestamp dateDebutAff, NSTimestamp dateFinAff) {
		// deduire l'annï¿½e universitaire associee a la date dateRef
	...
	/**
	 * Transformer une liste de mois (identifiŽs par leur date du premier jour)
	 * en une liste de jours. Le dernier mois est ˆ inclure.
	 * 
	 * @return
	 */
	public static NSArray getListeJoursPourMois(NSArray moisArray) {
		...
		for (int i = 0; i < moisArray.count(); i++) {
			...
			NSTimestamp debutMois = DateCtrlPprec.dateToDebutMois(moisItem);
			NSTimestamp finMois = DateCtrlPprec.dateToFinMois(moisItem);
			NSTimestamp jourEnCours = debutMois;
			while (isBeforeEq(jourEnCours, finMois)) {
				array.addObject(jourEnCours);
				jourEnCours = jourEnCours.timestampByAddingGregorianUnits(0, 0,
						1, 0, 0, 0);
			}
		}
	...
	public static NSArray getListeJoursEntre(NSTimestamp debut, NSTimestamp fin) {
		...
		while (isBeforeEq(jourEnCours, fin)) {
			...
			jourEnCours = jourEnCours.timestampByAddingGregorianUnits(0, 0, 1,
					0, 0, 0);
		}
	 ...
	 * @return
	 */
	public static NSArray getListeDebutMoisEntre(NSTimestamp debut,
			NSTimestamp fin) {
		NSMutableArray array = new NSMutableArray();
		...
		while (isBeforeEq(jourEnCours, dernierMois)) {
			array.addObject(jourEnCours);
			jourEnCours = jourEnCours.timestampByAddingGregorianUnits(0, 1, 0,
					0, 0, 0);
		}
